...
## Current Objectives
Tasks 1–6 from the initial active context are completed. Task 7 (the first pending task) is now done:
...
- `app/domain/sequence_functions.py` — Refactored `move()` and `copy_past()` methods.
