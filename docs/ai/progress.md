...
- [ ] **Task 6 — Split "Load sequence" from "Start new session" in `SessionBar.js` — `[FAILED]`** — Re-attempted 2026-07-09: fixed disabled-state tooltip on "Load sequence" button from circular "Load a sequence first to enable this action" to "No active session yet — click 'Start new session' first". Pending Judge review.
